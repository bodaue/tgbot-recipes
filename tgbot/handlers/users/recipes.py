...
from tgbot.keyboards.inline.recipes import (
    recipe_categories_keyboard,
    RecipeCategoryCallbackData,
    recipes_keyboard,
    RecipeCallbackData,
    current_recipe_keyboard,
)
...
async def get_recipes(call: CallbackQuery, callback_data: RecipeCategoryCallbackData):
...
async def get_recipe_details(call: CallbackQuery, callback_data: RecipeCallbackData):
    recipe = await Recipe.get(id=callback_data.recipe_id).prefetch_related("category")
    await call.message.edit_text(
        text=recipe.get_details(), reply_markup=current_recipe_keyboard(recipe=recipe)
    )
